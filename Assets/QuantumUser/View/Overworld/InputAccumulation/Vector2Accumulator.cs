namespace Quantum
{
	...
	public sealed class Vector2Accumulator
	{
		...
		{
			Vector2 consumeValue = _accumulatedValue;
		...
		public Vector2 ConsumeFrameAligned(QuantumGame game)
		{
			int currentFrame = Time.frameCount;

			// Revert accumulated value to state before latest accumulation.
			Vector2 consumeValue = _accumulatedValue - _unprocessedValue;
			...
			float baseAlpha = _lastConsumeFrame != currentFrame ? game.InterpolationFactor : 0.0f;

			// Here we calculate delta time between last Render time (or last tick aligned time) and time of the pending simulation tick.
			float tickAlignedDeltaTime = (1.0f - baseAlpha) * (float)game.Session.DeltaTimeDouble;

			// The unprocessed value is usually not aligned with ticks, we need to remove delta which is ahead of fixed tick time.
			Vector2 tickAlignedValue = _unprocessedValue * Mathf.Clamp01(tickAlignedDeltaTime / _unprocessedDeltaTime);
